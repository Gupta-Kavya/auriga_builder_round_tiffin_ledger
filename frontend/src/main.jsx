...
function App() {
  ...
  const [transfer, setTransfer] = useState({ name: '', phone: '', address: '', effectiveDate: today() });
  ...
  const transferSubscription = async event => {
    event.preventDefault(); setLoading(true); setMessage('');
    try {
      const data = await api(`/customers/${selected._id}/transfer`, token, { method: 'POST', body: JSON.stringify(transfer) });
      setSelected(data); setTransfer({ name: '', phone: '', address: '', effectiveDate: today() });
      await load(); setMessage('Subscription transferred. The monthly bill is split by recipient.');
    } catch (error) { alertError(error); } finally { setLoading(false); }
  };
  const importCustomers = async event => {
    event.preventDefault(); setLoading(true); setMessage('');
    try {
      const data = await api('/customers/import', token, { method: 'POST', body: JSON.stringify({ csv }) });
      setReport(data); await load(); setMessage('Import finished.');
    } catch (error) { alertError(error); } finally { setLoading(false); }
  };
  const notifyToday = async () => {
    setLoading(true); setMessage('');
    try {
      const result = await api('/clock', token, { method: 'POST', body: JSON.stringify({ date: notificationDate }) });
      const items = await api(`/outbox?date=${notificationDate}`, token);
      setOutbox(items); setMessage(`${result.notified} new delivery notifications queued for ${notificationDate}.`);
    } catch (error) { alertError(error); } finally { setLoading(false); }
  };
  ...
  return <>
    ...
    <div className="dashboard-grid twist-grid">
      <section className="panel"><div className="panel-head"><div><h2>Morning notifications</h2><p>Queue one message for each active weekday delivery.</p></div></div>
        <div className="actions"><input aria-label="Notification date" type="date" value={notificationDate} onChange={e => setNotificationDate(e.target.value)}/><button className="secondary" disabled={loading} onClick={notifyToday}>Notify due customers</button></div>
        <div className="pause-history">{outbox.map(item => <div key={item.customerId}>{item.phone} · {item.message}</div>)}</div>
      </section>
      <section className="panel"><div className="panel-head"><div><h2>Import customers</h2><p>Paste CSV with name,phone,address,monthlyPrice,startDate.</p></div></div>
        <form onSubmit={importCustomers} className="add-form"><label>Customer CSV<textarea required rows="5" placeholder={'name,phone,address,monthlyPrice,startDate\nPriya,9876543210,Main Street,2200,17/09/2026'} value={csv} onChange={e => setCsv(e.target.value)}/></label><button className="primary" disabled={loading}>Import list →</button></form>
        {report && <p className="hint">Imported: {report.imported} · Duplicates: {report.deduped} · Rejected: {report.rejected}{report.errors.length > 0 && ` · Invalid rows: ${report.errors.map(e => e.row).join(', ')}`}</p>}
      </section>
    </div>{selected && <section className="panel detail"><div className="panel-head"><div><p className="eyebrow">SUBSCRIPTION DETAIL</p><h2>{selected.name} <span className={`status ${selected.status}`}>{selected.status}</span></h2><p>{selected.phone} · {selected.address}</p></div><button className="close" onClick={() => setSelected(null)}>✕</button></div><div className="detail-grid"><div><div className="summary"><span>Monthly plan<strong>{money(selected.monthlyPrice)}</strong></span><span>Started<strong>{selected.startDate}</strong></span></div><h3>Pause deliveries</h3><p className="hint">Start and end dates are included. Leave end date empty until the customer resumes.</p><div className="two"><label>From<input type="date" value={pause.startDate} onChange={e => setPause({ ...pause, startDate: e.target.value })}/></label><label>Through (optional)<input type="date" value={pause.endDate} onChange={e => setPause({ ...pause, endDate: e.target.value })}/></label></div><div className="actions"><button className="secondary" disabled={loading} onClick={() => changePause('pause', pause)}>Save pause</button>{selected.pauses.some(p => !p.endDate) && <button className="secondary" disabled={loading} onClick={() => changePause('resume', { resumeDate: today() })}>Resume today</button>}</div><h3>Pause history</h3><div className="pause-history">{selected.pauses.length ? selected.pauses.map(p => <div key={p._id}>{p.startDate} → {p.endDate || 'ongoing'}</div>) : 'No pauses yet'}</div><h3>Transfer this plan</h3><p className="hint">The price and start date carry over. Billing splits on the first day served to the new customer.</p>
        <form onSubmit={transferSubscription} className="add-form"><div className="two"><label>New name<input required value={transfer.name} onChange={e => setTransfer({ ...transfer, name: e.target.value })}/></label><label>New phone<input required value={transfer.phone} onChange={e => setTransfer({ ...transfer, phone: e.target.value })}/></label></div><label>New address<input required value={transfer.address} onChange={e => setTransfer({ ...transfer, address: e.target.value })}/></label><label>Effective date<input required type="date" max={today()} value={transfer.effectiveDate} onChange={e => setTransfer({ ...transfer, effectiveDate: e.target.value })}/></label><button className="secondary" disabled={loading}>Transfer subscription</button></form>
        <div className="pause-history">{selected.holders?.map(h => <div key={h._id}>{h.name}: {h.startDate} → {h.endDate || 'current'}</div>)}</div>
      </div><div className="bill-card"><div className="bill-head"><div><p className="eyebrow">MONTHLY BILL</p><h3>What to charge</h3></div><input aria-label="Billing month" type="month" value={month} onChange={e => setMonth(e.target.value)}/></div>{bill && <><div className="bill-amount">{money(bill.amount)}</div><div className="bill-lines"><div><span>Weekdays in month</span><strong>{bill.weekdays}</strong></div><div><span>Days delivered</span><strong>{bill.deliveredDays}</strong></div><div><span>Paused weekdays</span><strong>{bill.pausedDays}</strong></div><div><span>Plan price</span><strong>{money(bill.monthlyPrice)}</strong></div>{bill.splits?.length > 1 && <div className="bill-splits"><h3>Split by customer</h3>{bill.splits.map(part => <div key={`${part.phone}-${part.name}`}><span>{part.name} · {part.deliveredDays} days</span><strong>{money(part.amount)}</strong></div>)}</div>}</div><p className="bill-note">{bill.provisional ? `Provisional through ${bill.through || 'before month start'}. ` : ''}Plan price ÷ weekdays in month × delivered days. Weekends are excluded.</p></>}</div></div></section>}</main>}
